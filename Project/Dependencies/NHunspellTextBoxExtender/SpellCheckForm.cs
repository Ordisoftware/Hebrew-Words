...
namespace NHunspellExtender
{
  ...
  public partial class SpellCheckForm
  {
    ...
    private void SpellCheckForm_Shown(object sender, EventArgs e)
    {
      ...
      {
        if ( string.IsNullOrEmpty(_originalTextBox.Text) | !_spellChecker.HasSpellingErrors() )
          Hide();
      }
      else if ( string.IsNullOrEmpty(_originalRichTextBox.Text) | !_spellChecker.HasSpellingErrors() )
        Hide();
    }
    ...
    private void cmdAdd_Click(object sender, EventArgs e)
    {
      if ( !_DisableConfirmationPrompt )
      {
        ...
        if ( (int)dlgResult == (int)DialogResult.Ignore + (int)DialogResult.No )
        {
          _DisableConfirmationPrompt = true;
          return;
        }
        else if ( (int)dlgResult == (int)DialogResult.Yes + (int)DialogResult.Ignore )
        {
          _DisableConfirmationPrompt = true;
        }
      ...
      // TODO _spellChecker.myNHunspell.RootWords.Add(currentWord);
      ...
      if ( _callingTextBox is RichTextBox )
      {
        ...
        ( (RichTextBox)_callingTextBox ).ZoomFactor = 1f;
        ( (RichTextBox)_callingTextBox ).ZoomFactor = (float)_zoomFactor;
      }
      else
      {
    ...
    private void cmdIgnoreAll_Click(object sender, EventArgs e)
    {
      ...
      if ( dlgResult == DialogResult.Yes )
      {
        ...
        string currentWord = Strings.Mid(txtCurrentSentence.Text, currentWordRange.First + 1, currentWordRange.Length);
        // TODO _spellChecker.myNHunspell.Add(currentWord);
    ...
    private void cmdChange_Click(object sender, EventArgs e)
    {
      ...
      int newSentenceLength = txtCurrentSentence.TextLength;
    ...
    private void cmdChangeAll_Click(object sender, EventArgs e)
    {
      ...
      string originalWord = Strings.Mid(textBoxToUse.Text, (int)( currentSentenceStartIndex + currentWordRange.First + 1L ), currentWordRange.Length);
      string newWord;
      long originalSentenceLength = sentenceBreaks[1, (int)currentSentence] - currentSentenceStartIndex + 1L;
      int newSentenceLength = txtCurrentSentence.TextLength;
      ...
      foreach ( var currentRange in _spellChecker.GetSpellingErrorRanges() )
      {
    ...
    public void SetSentenceBreaks()
    {
      ...
      for ( int i = 0, loopTo = textBoxToUse.TextLength - 2; i <= loopTo; i++ )
      {
        if ( Conversions.ToBoolean(IsSentenceEnding(textBoxToUse.Text[i])) )
        {
          ...
          sentenceBreaks[0, Information.UBound(sentenceBreaks, 2)] = i;
          sentenceBreaks[1, Information.UBound(sentenceBreaks, 2)] = i;
        }
      }
    ...
    public bool SetNextWord()
    {
      ...
      var ranges = _spellChecker.GetSpellingErrorRanges();
      DisableTextChanged = true;
          ...
          for ( int i = 0, loopTo = Information.UBound(sentenceBreaks, 2); i <= loopTo; i++ )
          {
            if ( i == 0 )
            {
              if ( sentenceBreaks[1, i] > currentWordStart )
              {
                ...
                txtCurrentSentence.Text = Strings.Left(textBoxToUse.Text, (int)( sentenceBreaks[0, i] + 1L ));
                ...
                txtCurrentSentence.ZoomFactor = 1f;

                // Now set up the suggestions box
                txtCurrentSentence.ZoomFactor = (float)_zoomFactor;
                lboSuggestions.Items.Clear();

                foreach ( var st in _spellChecker.GetSuggestions(Strings.Mid(textBoxToUse.Text, currentRange.First + 1, currentRange.Length), 10) )
