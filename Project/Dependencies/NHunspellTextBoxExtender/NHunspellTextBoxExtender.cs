...
namespace NHunspellExtender
{
  ...
  public class NHunspellTextBoxExtender : Component, IExtenderProvider, ISupportInitialize
  {
    ...
    private class CustomPaintTextBox : NativeWindow
    {
      ...
      private void CustomPaint() // ByVal sender As Object, ByVal e As DoWorkEventArgs)
      {
        ...
        for ( int i = 0, loopTo = Information.UBound(CharRanges); i <= loopTo; i++ )
        {
          ...
          foreach ( var currentIgnoreRange in mySpellCheckControl.GetIgnoreRanges() )
          {
          ...
          if ( !ignoreRange )
          {
            for ( int j = CharRanges[i].First, loopTo1 = CharRanges[i].First + CharRanges[i].Length - 1; j <= loopTo1; j++ )
            {
              {
        ...
        CustomPaintComplete?.Invoke(parentTextBox, (long)Math.Round(DateTime.Now.Subtract(startTime).TotalMilliseconds));
      }
      ...
      private Point GetOffsets(ref TextBoxBase curTextBox, int startingIndex, int endingIndex, RichTextBox tempRTB = null)
      {
        ...
        int offsetX = 0;
        offsetX = (int)Math.Round(textBoxGraphics.MeasureString(Conversions.ToString(curTextBox.Text[startingIndex + ( endingIndex - startingIndex )]), fontToUse).Width);
    ...
    //public string CurrentLangAffFileName = "fr_FR.aff";
    //public string CurrentLangDicFileName = "fr_FR.dic";
    //public byte[] CurrentLangAff = My.Resources.Resources.en_US;
    //public byte[] CurrentLangDic = My.Resources.Resources.en_US_dic;
    //public string CurrentLangAffFileName = "en_US.aff";
    //public string CurrentLangDicFileName = "en_US.dic";
    ...
    public NHunspellTextBoxExtender()
    {
      ...
      string pathApp = Ordisoftware.Core.Globals.DictionariesFolderPath;
      string pathUser = Ordisoftware.Core.Globals.UserDataFolderPath;
      //Directory.CreateDirectory(pathUser);

      USaff = Path.Combine(pathApp, Languages.CurrentCode + ".aff");
      USdic = Path.Combine(pathApp, Languages.CurrentCode + ".dic");
    ...
    //  //USdic = callingDir + @"\SpellCheck\" + CurrentLangDicFileName;
    //  //USaff = callingDir + @"\SpellCheck\" + CurrentLangAffFileName;
    ...
    //  //  try
    //  //  {
    //  //    File.WriteAllBytes(USaff, CurrentLangAff);
    //  //  }
    //  //  catch ( Exception ex )
    //  //  {
    ...
    //  //  try
    //  //  {
    //  //    File.WriteAllBytes(USdic, CurrentLangDic);
    //  //  }
    //  //  catch ( Exception ex )
    //  //  {
    ...
    CreateNewHunspell:
      ...
      if ( !Ordisoftware.Core.Globals.IsVisualStudioDesigner )
        try
        {
          ...
          if ( File.Exists(Path.Combine(pathUser, $"CustomWords-{Languages.Current}.dic")) )
            using ( var r = new StreamReader(Path.Combine(pathUser, $"CustomWords-{Languages.Current}.dic")) )
              while ( !r.EndOfStream )
        ...
        catch ( Exception ex )
        {
          if ( ex is DllNotFoundException )
          {
            ...
            //  try
            //  {
            //    File.WriteAllBytes(DLLpath, My.Resources.Resources.Hunspellx64);
            //  }
            //  catch ( Exception ex2 )
            //  {
            ...
            //  try
            //  {
            //    File.WriteAllBytes(DLLpath, My.Resources.Resources.Hunspellx86);
            //  }
            //  catch ( Exception ex3 )
            //  {
            ...
            //  try
            //  {
            //    File.WriteAllBytes(DLLpath, My.Resources.Resources.NHunspell);
            //  }
            //  catch ( Exception ex4 )
            //  {
    ...
    // Try
    // File.WriteAllBytes(USaff, CurrentLang)
    ...
    {
      if ( controlEnabled[extendee] is null )
      {
    ...
    public void SetSpellCheckEnabled(Control extendee, bool Input)
    {
      ...
      if ( controlEnabled[extendee] is null )
      {
        ...
        SpellCheckControl argThisSpellCheckControl = (SpellCheckControl)mySpellCheckers[extendee];
        var argParent = this;
      ...
      {
        controlEnabled[extendee] = Input & myNHunspell is not null;
      }
    ...
    {
      if ( (int)ShortcutKey == (int)e.KeyCode )
      {
    ...
    private void TextBox_KeyPress(object sender, KeyPressEventArgs e)
    {
...

      {
        ...
        {
          withBlock.AddText(Conversions.ToString(e.KeyChar), ( (TextBoxBase)sender ).SelectionStart);
        }
    ...
    public void ContextMenuItem_Click(object sender, EventArgs e)
    {
      ...
      switch ( ( (ToolStripMenuItem)sender ).Name ?? "" )
      {
        ...
        case "qwr3Spell1":
          {
            ReplaceWord(Conversions.ToString(Suggestion1.Tag), Suggestion1.Text);
            break;
          }
        case "qwr3Spell2":
          {
            ReplaceWord(Conversions.ToString(Suggestion2.Tag), Suggestion2.Text);
            break;
          }
        case "qwr3Spell3":
          {
            ReplaceWord(Conversions.ToString(Suggestion3.Tag), Suggestion3.Text);
            break;
          }
        case "qwr3Spell4":
          {
            ReplaceWord(Conversions.ToString(Suggestion4.Tag), Suggestion4.Text);
            break;
          }
        case "qwr3Spell5":
          {
            ReplaceWord(Conversions.ToString(Suggestion5.Tag), Suggestion5.Text);
            break;
          }
        case "qwr3Add":
          {
            AddWordToDictionary(Conversions.ToString(AddWord.Tag));
            break;
          }
        case "qwr3Ignore":
          {
            IgnoreSelectedWord(Conversions.ToString(IgnoreWord.Tag), ( (ContextMenuStrip)( (ToolStripMenuItem)sender ).Owner ).Left, ( (ContextMenuStrip)( (ToolStripMenuItem)sender ).Owner ).Top);
            break;
          }
        case "qwr3IgnoreAll":
          {
            IgnoreAllWord(Conversions.ToString(IgnoreAll.Tag));
            break;
          }
        case "qwr3Spelling":
          {
            ...
            {
              if ( Conversions.ToBoolean(Operators.ConditionalCompareObjectEqual(myControls[i].Name, Spelling.Tag, false)) )
              {
    ...
    internal void ContextMenu_Opening(object sender, CancelEventArgs e)
    {
      ...
      if ( ownerTextBox is RichTextBox )
      {

        {
          ...
          using ( var g = withBlock1.CreateGraphics() )
          {
            int y;
            y = (int)Math.Round(g.MeasureString(Conversions.ToString(withBlock1.GetCharFromPosition(currentMouseLocation)), selFont).Height + withBlock1.GetPositionFromCharIndex(charIndex).Y);
      ...
      else
      {
        ...
        using ( var g = ownerTextBox.CreateGraphics() )
        {
          int y;
    ...
    private void ReplaceWord(string OriginalWord, string NewWord)
    {
      ...
      {
        var withBlock = (SpellCheckControl)mySpellCheckers[ownerTextBox];
        charRanges = withBlock.GetSpellingErrorRanges();
      }
      ...
      if ( ownerTextBox is RichTextBox )
      {
        double _zoomFactor = (double)( (RichTextBox)ownerTextBox ).ZoomFactor;
        ...
        ( (RichTextBox)ownerTextBox ).ZoomFactor = 1f;
        ( (RichTextBox)ownerTextBox ).ZoomFactor = (float)_zoomFactor;
      }
      else if ( wordStart == 0 )
      {
    ...
    private void AddWordToDictionary(string WordToAdd)
    {
      if ( !boolDisableAddWordPrompt )
      {
        ...
        if ( (int)result == (int)DialogResult.Ignore + (int)DialogResult.No )
        {
          boolDisableAddWordPrompt = true;
          return;
        }
        else if ( (int)result == (int)DialogResult.Yes + (int)DialogResult.Ignore )
        {
          boolDisableAddWordPrompt = true;
        }
      ...
      using ( var w = new StreamWriter(Path.Combine(Ordisoftware.Core.Globals.UserDataFolderPath, $"CustomWords-{Languages.Current}.dic"), true) )
      {
    ...
    private void IgnoreSelectedWord(string callingTextBoxName, int LeftLocation, int TopLocation)
    {
      ...
      foreach ( var currentRange in ( (SpellCheckControl)mySpellCheckers[callingTextBox] ).GetSpellingErrorRanges() )
      {
    ...
    //        var withBlock = (TextBoxBase)myControls[i];
    //        {
    //          var withBlock1 = (SpellCheckControl)mySpellCheckers[myControls[i]];
    ...
    public void RunFullSpellChecker(ref TextBoxBase callingTextBox)
    {
      ...
      if ( callingTextBox is RichTextBox )
      {
        ...
        zoomFactor = (double)( (RichTextBox)callingTextBox ).ZoomFactor;
      }

      // Create the new spell checking form
      var newSpellCheckForm = new SpellCheckForm(ref callingTextBox, (SpellCheckControl)mySpellCheckers[callingTextBox], boolDisableAddWordPrompt);
      ...
      {
        {
          ...
          withBlock.ZoomFactor = 1f;
          withBlock.ZoomFactor = (float)zoomFactor;
        }
      }
